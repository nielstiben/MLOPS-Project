...
We intend to use pre-trained models due to limited time, and also train the model(s) additionally on our dataset. Since we are working on tweets then one of the models we plan to use is the BERTweet model which is the first public large-scale pre-trained language model for English Tweets (https://huggingface.co/docs/transformers/model_doc/bertweet).
